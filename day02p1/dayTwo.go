...
func processDay2Line(line string, ballType int) bool {
	...
	return value <= ballCount[ballType]
}
...
func mainTwoPOne() {
	...
	for index, line := range linesOfText {
		...
		if processDay2Line(line, 0) && processDay2Line(line, 1) && processDay2Line(line, 2) {
			fmt.Println(`Adding `, index+1)
			sum += index + 1
		}
	}
